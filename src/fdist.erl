...
-export([fdistpdf/2, fdistcdf/2, fdistinv/2]).
...
% ------------------------------------------------------------------------------
%  fdistpdf - Uniform probability density function
% ------------------------------------------------------------------------------
fdistpdf(_,Nu) when Nu =< 0 -> {error,"Nu parameter =< 0 in Chi-square."};

fdistpdf(X,_) when X < 0 -> 0.0;
fdistpdf(X,Nu) ->
        Fac = 0.693147180559945309*(0.5*Nu) + gammaln(0.5*Nu),
        math:exp(-0.5*(X-(Nu-2)*math:log(X))-Fac).

% ------------------------------------------------------------------------------
%  fdistcdf - Uniform cumulative distribution function
% ------------------------------------------------------------------------------
fdistcdf(_,Nu) when Nu =< 0 -> {error,"Nu parameter =< 0 in Chi-square."};

fdistcdf(X,_) when X < 0 -> 0.0;
fdistcdf(X,Nu) ->
        gammap(0.5*Nu,0.5*X).

        
% ------------------------------------------------------------------------------
%  fdistinv - Inverse chisq distribution function
% ------------------------------------------------------------------------------
fdistinv(_,Nu) when Nu =< 0 -> {error,"Nu parameter =< 0 in Chi-square."};

fdistinv(P,_) when P < 0 orelse P > 1 -> {error,"Invalid probability"};

fdistinv(X,_) when X =< 0 -> 0.0;
fdistinv(P,Nu) ->
...
fdistpdf_test() ->
        ?assertEqual(0.0, fdistpdf(-1.0,1)),
        ?assertEqual(0.24197072451914325, fdistpdf(1,1)),
        ?assertEqual(0.10377687435514864, fdistpdf(2,1)),
        ?assertEqual(0.3032653298563167, fdistpdf(1,2)),
        ?assertEqual(0.003368973499542733, fdistpdf(10,2)),
        ?assertEqual(0.00766415502440505, fdistpdf(2,10)).

fdistpdf_error_test() ->
        ?assertEqual({error,"Nu parameter =< 0 in Chi-square."},  fdistpdf(0.0,-1)).

fdistcdf_test() ->
        ?assertEqual(0.0, fdistcdf(-1.0,1)),
        ?assertEqual(0.6826894921370856, fdistcdf(1,1)),
        ?assertEqual(0.8427007929497147, fdistcdf(2,1)),
        ?assertEqual(0.3934693402873665, fdistcdf(1,2)),
        ?assertEqual(0.9932620530009145, fdistcdf(10,2)),
        ?assertEqual(0.0036598468273437135, fdistcdf(2,10)).

fdistcdf_error_test() ->
        ?assertEqual({error,"Nu parameter =< 0 in Chi-square."},  fdistcdf(0.0,-1)).

fdistinv_test() ->
        ?assertEqual(0.0, fdistinv(0.0,1)),
        ?assertEqual(0.015790774093431218, fdistinv(0.1,1)),
        ?assertEqual(0.2107210313156527, fdistinv(0.1,2)),
        ?assertEqual(0.45493642311957305, fdistinv(0.5,1)),
        ?assertEqual(1.3862943611198906, fdistinv(0.5,2)),
        ?assertEqual(15.987179172105261, fdistinv(0.9,10)).
